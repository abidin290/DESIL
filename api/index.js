...
async function uploadPhoto(r,user,name){
  ...
  let bytes;try{bytes=Buffer.from(String(r.data||''),'base64');}catch(_){fail('Data foto tidak valid.');}
...
async function complete(r,user,name){
  ...
  for(let i=0;i<5;i++){const f=await getFile(r.ids[i+1]);if(!f||f.trashed||f.name!==PHOTO_NAMES[i]||!(f.parents||[]).includes(r.ids[0]))fail('Laporan belum lengkap. Lanjutkan upload foto yang belum diterima.',409,true);}
...
module.exports=async function handler(req,res){
  ...
  try{return res.status(200).json(await handle(req.body||{}));}
