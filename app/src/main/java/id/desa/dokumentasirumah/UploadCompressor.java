...
final class UploadCompressor {
    ...
    File prepare(File source,LocationStamp location,String... header)throws Exception{
        ...
        StringBuilder key=new StringBuilder(location==null?"standard1-":"standard3gps-");for(byte b:digest.digest())key.append(String.format(java.util.Locale.ROOT,"%02x",b&255));
        ...
        try(InputStream in=dest.openRead()){if(result.length()>0 && result.length()<=MAX_BYTES)return result;}catch(FileNotFoundException ignored){}
